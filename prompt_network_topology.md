...
- **Visibilidade de cross-cloud egress por par**: `conn-azure-to-aws-pmntengine` = R$ 90,48 / 210 GB tem semântica de negócio diferente de R$ 90 de internet pública (payment engine híbrido vs egress genérico). Sem split, vira ruído no nó Internet.
...
- **Atribuição de custo a workload**: o `ResourceGroup` da connection (extraído do `ResourceId`) normalmente identifica o sistema dono (`rg_pmnt_databases_prd`), permitindo chargeback per-business-unit.
...
- Fatura **sem** linhas `microsoft.network/connections/` em ResourceId → `_by_connection` fica vazio, acumuladores ficam zerados. Comportamento padrão para enrollments puramente Azure-interno (ER doméstico, peering intra-Azure). Ex: Petrobras 2026-04 — 0 connections faturadas.
...
- **Atribuíção cirúrgica de custo**: TI-Infra-CSC (Raizen 2026-04) tem 3 circuitos com SKUs distintos. Custo agregado `expressroute_out_cost = R$ 11.344` não revela que **um único circuito** (`er-infra-equinix-sp3-eus2`) responde por ~100% dele (16.54 TB) enquanto outro (`expressroute-infra-csc-02-sp4`) processa 240.96 TB de ingress (GRÁTIS). Sem split, o analista não sabe qual circuito reservar para upsizing/downsizing.
...
- `[AUTO-RESOURCE]` durante extração quando Path B dispara:
  ...
  [AUTO-RESOURCE] inferred rtype='expressroute_gateway' via=flow
      ...
      (example resource: vng-hub-gru-br)
  ```
- `[AUTO-FLOW]` durante extração quando Path D dispara:
  ...
  [AUTO-FLOW] inferred flow_key='public_ip_cost' from rtype='public_ip'
      ...
      (example resource: pip-bastion-prd-001)
  ```
...
O caso canônico que motivou esse eixo: **VPN Connection tráfego** (§1.1.5). A linha tem `MeterCategory = "Bandwidth"`, `MeterName = "Standard Data Transfer Out"` — indistinguível em billing de qualquer outro egress — mas o `ResourceId` aponta para `/providers/microsoft.network/connections/conn-azure-to-aws-pmntengine`. **Nenhum classifier baseado em MeterName conseguiria separar essa linha das demais**; só o path ARM revela a semântica.
...
def parse_arm_path(rid: str) -> dict | None:
    ...
    return {
        ...
        'name': m.group(3) or '',            # 'conn-azure-to-aws-pmntengine'
...
3. Se tem **ambos** → tentar inferir pelo nome (`conn-*vpn*`, `conn-*er*`, `conn-*ipsec*`, `conn-*aws*`/`*gcp*`/`*oci*` ⇒ VPN; `conn-*megaport*`, `conn-*equinix*` ⇒ ER). Se inconclusivo → `rtype = "connection"` genérico + log `[CONNECTION-AMBIG]`.
...
**Dynamic cols rationale**: em datasets com regiões muito desiguais (ex: Raizen tem 1 região com 37 subs e outras com 3), forçar 2 colunas em toda a tela gerava region box com 19 linhas (h ≈ 2.973 px). Com `colsFor(37) = 5`, a mesma região cai para 8 linhas (h ≈ 1.433 px = −52%). A função:
...
4. **Se falha**: o agente deve **PARAR** e apresentar ao usuário:
   - Quais clientes deviam (Riachuelo / CEMIG / outros)
...
- **Caso real**: enrollment Renner (2026-04) tem 3 ER Gateways com SKU `Standard Gateway` — `vng-hub-gru-br`, `vng-rfid-infra-er`, `vnet-gw-oms-expressroute_migrated`.
...
- **Sintoma**: ao adicionar SKU "Standard Gateway" ao pattern de `expressroute_gateway` (sem whitelist), a contagem de **NAT Gateway** despencou em todos os 3 clientes (Riachuelo: -8, Renner: -14, CEMIG: -2) e a contagem de **ER Gateway** subiu na mesma proporção.
...
- **Caso real**: detectado em todos os 3 clientes — Renner R$ 4.404, Riachuelo R$ 2.539, CEMIG R$ 575 de Public IPs sem flow.
...
- **Caso real**: fatura Petrobras 2026-04 — 24 linhas com `MeterCategory = "Azure Bastion"`, `MeterName = "Basic Gateway"`.
...
#### A.14 ResourceId aponta para serviço-pai (AML, Databricks, VMs) — não para componente de rede
...
- **Caso real** (fatura 2026-04): 90.205 linhas com `MeterRegion = "Intercontinental"` e `ResourceLocation = "brazilsouth"` — representa egress de BR para outros continentes, comportamento esperado.
...
- **Caso real** (Riachuelo 2026-04): 63 linhas `Bandwidth / Standard Data Transfer Out` com `ResourceId = /...providers/microsoft.network/connections/conn-azure-to-aws-pmntengine`, R$ 90,48 / 210 GB egress + R$ 0 / 160 GB ingress. Workload: payment engine híbrido Azure DB ↔ AWS via VPN S2S. Cenário oposto em Petrobras 2026-04: zero linhas com esse padrão (ER doméstico não emite billing per-connection).
...
- **Insight de negócio**: nome da connection (`conn-azure-to-aws-pmntengine`) + ResourceGroup (`rg_pmnt_databases_prd`) frequentemente identifica o sistema dono e o destino remoto. Permite chargeback per-workload e detecção precoce de growth cross-cloud (custo unitário é baixo — R$ 0,43/GB típico Brazil South — mas escala linear com volume).
